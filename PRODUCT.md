...
- The screenshot lightbox grows the tapped/clicked image dramatically larger (up to 96vw/93vh, using a sized frame + `object-fit: contain` so a small source image is actually upscaled rather than just capped) with a scale-in grow animation on open; prev/next/close controls use a darker blurred background so they stay legible over any photo content.
...
- Screenshots: Gameory now shows 3 real in-app screenshots (`public/screenshots/gameory-*.jpg`) in the fanned stack, no "MOCKUP" label. The other three apps still show the wireframe placeholder (clearly labeled "MOCKUP") until their real screenshots are provided — `OryApp.screenshots` in `src/data/apps.ts` is optional per app; the fan and the "MOCKUP" tag switch automatically based on whether it's set.
